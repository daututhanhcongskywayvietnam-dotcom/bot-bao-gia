...
# Nếu không thấy file ở ngoài, Bot sẽ tự tìm trong két sắt của Render (/etc/secrets/)
if os.path.exists('google_key.json') and os.path.getsize('google_key.json') > 10:
...
def ghi_google_sheet(user_name, text_content, current_rate):
    try:
        ...
        try: sh = client.open(SHEET_NAME)
            ...
            print(f"❌ Lỗi: Không mở được file Sheet '{SHEET_NAME}'. Kiểm tra lại Tên hoặc Quyền chia sẻ.")
            return
