...
# --- [MỚI] THÊM THƯ VIỆN GOOGLE SHEET ---
import gspread
...
# CẤU HÌNH SHEET (Đúng tên Doàng)
SHEET_NAME = "Doàng Thu USDT - 2026" 
WORKSHEET_NAME = "Bán SWC"

# --- [QUAN TRỌNG] TỰ ĐỘNG TÌM FILE KEY TRONG KÉT SẮT RENDER ---
# Bot sẽ tự tìm file key dù bạn để ở đâu
if os.path.exists('google_key.json'):
    CREDENTIALS_FILE = 'google_key.json'
else:
    CREDENTIALS_FILE = '/etc/secrets/google_key.json'

# NỘI DUNG CHUYỂN KHOẢN (Giữ nguyên của bạn)
NOI_DUNG_CK = """
✅ **NGÂN HÀNG:** ACB
...
# --- [MỚI] HÀM GHI GOOGLE SHEET ---
def ghi_google_sheet(user_name, text_content, current_rate):
    """Hàm chạy ngầm để ghi dữ liệu vào Sheet"""
    try:
        # 1. Kết nối
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        
        # Kiểm tra file key (Code cũ của bạn thiếu đoạn này nên Bot không tìm thấy file trên Render)
        if not os.path.exists(CREDENTIALS_FILE): 
            print(f"⚠️ Lỗi: Không tìm thấy file key tại {CREDENTIALS_FILE}")
            return
        ...
        try:
            ...
            print(f"❌ Lỗi: Không tìm thấy file '{SHEET_NAME}'. Hãy kiểm tra lại tên file hoặc quyền chia sẻ.")
            return
        ...
        # 4. Tìm dòng trống (Từ dòng 8 trở đi)
        ...
        print(f"✅ Đã ghi vào dòng {next_row}: {data}")
...
# [CHỈNH SỬA QUAN TRỌNG] Thêm biến text_content vào để lấy dữ liệu ghi Sheet
async def send_congrats(update, context, text_content):
    ...
    # [MỚI] GHI VÀO GOOGLE SHEET NGAY TẠI ĐÂY
    user_name = update.effective_user.first_name
    rate = bot_data.get("current_usd_rate", 27.0)
    # Chạy luồng riêng để Bot không bị đơ
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if has_photo and has_gmail and has_money:
        # [MỚI] Truyền thêm nội dung tin nhắn 'text' vào để ghi sheet
