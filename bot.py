...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👋 Chào mừng bạn! Nhắn số lượng USD để nhận báo giá.", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def welcome_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    for member in update.message.new_chat_members:
        ...
        keyboard = [[InlineKeyboardButton("🇻🇳 CÀI TIẾNG VIỆT", url="https://t.me/setlanguage/vi-beta")],
                    [InlineKeyboardButton("📢 XEM KÊNH TIN TỨC 🇻🇳", url=LINK_CHANNEL)]]
        await update.message.reply_text(f"👋 Chào {member.first_name}! Bạn hãy cài Tiếng Việt và theo dõi kênh tin tức tại đây 👇", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Chỉ cần có số là báo giá (Nhạy hơn bản cũ)
    if match:
        amount = int(match.group())
        if amount <= 0: return # Bỏ qua số 0
